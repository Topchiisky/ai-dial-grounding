...
SYSTEM_PROMPT = """
You are a helpful RAG-powered assistant that provides comprehensive answers based on user search results.
...
## INSTRUCTIONS:
1. Use the provided `RAG CONTEXT` as context when answering the `USER QUESTION`.
2. Cite specific sources when using information from the context.
3. Answer ONLY based on conversation history and RAG context.
4. If no relevant information exists in `RAG CONTEXT` or conversation history, state that you cannot answer the question.
5. Be conversational and helpful in your responses.
6. When presenting user information, format it clearly and include relevant details.
"""
...
class UserRAG:
    ...
    async def _create_vectorstore_with_batching(self, documents: list[Document], batch_size: int = 100):
        ...
        for batch in batches:
            ...
            batches_tasks.append(task)
        # 3. Gather tasks with asyncio
        batch_results = await asyncio.gather(*batches_tasks, return_exceptions=True)
    ...
    async def retrieve_context(self, query: str, k: int = 10, score: float = 0.1) -> str:
        ...
        print("🔎 Retrieving relevant documents from vectorstore...")
        ...
        for doc, relevance_score in relevant_docs:
            ...
            print(f" - [Score: {relevance_score}] {doc.page_content}")
